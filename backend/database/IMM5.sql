-- IMM5.sql
-- Fonctions pour les statistiques utilisées par ReportsPage
-- Ajouté : get_monthly_revenue, get_category_distribution,
-- get_top_products, get_hourly_orders, get_kpis, get_monthly_summary

-- Remarques :
-- - Les revenus et ventes sont calculés à partir des lignes de la table Contenir
--   (quantite * prix_unitaire) pour les menus dont Menu.restaurant_hote = restaurant_id
--   et pour des commandes qui ont un Paiement avec statut 'reussi'.
-- - Le taux de satisfaction est calculé à partir des notes de Noter (menus)
--   et des notes de Livraison (si disponibles). Le résultat est renvoyé en % (0-100).
